...
function addInputToList(index,edit) {
    ...
    var mappingStream = document.getElementById("mappingStream");
    var eventCalss = document.getElementById("eventClassName");
    ...
    addInputTopicToSession(topicName.value, brokerName.value, mappingStream.value, index, eventCalss.value, mappingType.value,edit);
    return true;
}
...
function addQueryToList(index,edit) {
    ...
    addQueryToSession(type, queryName.value, query.value, topicName.value, brokerName.value, outputMapping.value, xmlMappingText.value, index,edit);
    return true;
}
...
function addQueryToSession(type, queryName, sourceText, outputTopic, brokerName, outputMapping,
                                xmlMappingText, tableIndex,edit) {
    var callback =
    {
        success:function (o) {
            if (edit) {
    ...
    var request = YAHOO.util.Connect.asyncRequest('POST', "cep_add_query.jsp", callback, "type=" + type + "&queryName=" + queryName + "&sourceText=" + sourceText + "&outputTopic=" + outputTopic + "&brokerName=" + brokerName + "&xmlMappingText=" + xmlMappingText + "&tableIndex=" + tableIndex + "&outputMapping=" + outputMapping);
}
