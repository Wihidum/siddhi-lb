...
definitionStream returns [StreamDefinition streamDefinition]
	...
	:  ^(streamId {$streamDefinition.name($streamId.value);} (^(IN_ATTRIBUTE attributeName type  {$streamDefinition.attribute($attributeName.value, $type.type);}))+  (IP {$streamDefinition.ip($IP.text);})?)
	;
