...
definitionStream 
	:'define' 'stream' streamId ('@' IP)?  '(' attributeName type (',' attributeName type )* ')'  ->  ^(streamId  (^(IN_ATTRIBUTE attributeName type))+  IP?)
	;
...
IP : POSITIVE_INT_VAL '.' POSITIVE_INT_VAL '.' POSITIVE_INT_VAL '.' POSITIVE_INT_VAL    ;
