...
import org.wso2.siddhi.core.util.extensionholder.ConditionExtensionHolder;
import org.wso2.siddhi.core.util.extensionholder.ExpressionExtensionHolder;
...
public class ExecutorParser {
    ...
    public static ConditionExecutor parseCondition(Condition condition,
        ...
        if (condition instanceof AndCondition) {
            return new AndConditionExecutor(parseCondition(((AndCondition) condition).getLeftCondition(), queryEventStreamList, currentStreamReference, processInStreamDefinition, siddhiContext),
                                            parseCondition(((AndCondition) condition).getRightCondition(), queryEventStreamList, currentStreamReference, processInStreamDefinition, siddhiContext));
        } else if (condition instanceof OrCondition) {
            return new OrConditionExecutor(parseCondition(((OrCondition) condition).getLeftCondition(), queryEventStreamList, currentStreamReference, processInStreamDefinition, siddhiContext),
                                           parseCondition(((OrCondition) condition).getRightCondition(), queryEventStreamList, currentStreamReference, processInStreamDefinition, siddhiContext));
        } else if (condition instanceof NotCondition) {
            return new NotConditionExecutor(parseCondition(((NotCondition) condition).getCondition(), queryEventStreamList, currentStreamReference, processInStreamDefinition, siddhiContext));
        } else if (condition instanceof BooleanCondition) {
            return new BooleanConditionExecutor(parseExpression(((BooleanCondition) condition).getExpression(), queryEventStreamList, currentStreamReference, processInStreamDefinition, siddhiContext));
        } else if (condition instanceof Compare) {
            if (((Compare) condition).getOperator() == Condition.Operator.EQUAL) {
                return ComparatorParser.parseEqualCompare(parseExpression(((Compare) condition).getLeftExpression(), queryEventStreamList, currentStreamReference, processInStreamDefinition, siddhiContext),
                                                          parseExpression(((Compare) condition).getRightExpression(), queryEventStreamList, currentStreamReference, processInStreamDefinition, siddhiContext));
            } else if (((Compare) condition).getOperator() == Condition.Operator.NOT_EQUAL) {
                return ComparatorParser.parseNotEqualCompare(parseExpression(((Compare) condition).getLeftExpression(), queryEventStreamList, currentStreamReference, processInStreamDefinition, siddhiContext),
                                                             parseExpression(((Compare) condition).getRightExpression(), queryEventStreamList, currentStreamReference, processInStreamDefinition, siddhiContext));
            } else if (((Compare) condition).getOperator() == Condition.Operator.GREATER_THAN) {
                return ComparatorParser.parseGreaterThanCompare(parseExpression(((Compare) condition).getLeftExpression(), queryEventStreamList, currentStreamReference, processInStreamDefinition, siddhiContext),
                                                                parseExpression(((Compare) condition).getRightExpression(), queryEventStreamList, currentStreamReference, processInStreamDefinition, siddhiContext));
            } else if (((Compare) condition).getOperator() == Condition.Operator.GREATER_THAN_EQUAL) {
                return ComparatorParser.parseGreaterThanEqualCompare(parseExpression(((Compare) condition).getLeftExpression(), queryEventStreamList, currentStreamReference, processInStreamDefinition, siddhiContext),
                                                                     parseExpression(((Compare) condition).getRightExpression(), queryEventStreamList, currentStreamReference, processInStreamDefinition, siddhiContext));
            } else if (((Compare) condition).getOperator() == Condition.Operator.LESS_THAN) {
                return ComparatorParser.parseLessThanCompare(parseExpression(((Compare) condition).getLeftExpression(), queryEventStreamList, currentStreamReference, processInStreamDefinition, siddhiContext),
                                                             parseExpression(((Compare) condition).getRightExpression(), queryEventStreamList, currentStreamReference, processInStreamDefinition, siddhiContext));
            } else if (((Compare) condition).getOperator() == Condition.Operator.LESS_THAN_EQUAL) {
                return ComparatorParser.parseLessThanEqualCompare(parseExpression(((Compare) condition).getLeftExpression(), queryEventStreamList, currentStreamReference, processInStreamDefinition, siddhiContext),
                                                                  parseExpression(((Compare) condition).getRightExpression(), queryEventStreamList, currentStreamReference, processInStreamDefinition, siddhiContext));

            } else if (((Compare) condition).getOperator() == Condition.Operator.CONTAINS) {
                return ComparatorParser.parseContainsCompare(parseExpression(((Compare) condition).getLeftExpression(), queryEventStreamList, currentStreamReference, processInStreamDefinition, siddhiContext),
                                                             parseExpression(((Compare) condition).getRightExpression(), queryEventStreamList, currentStreamReference, processInStreamDefinition, siddhiContext));
            } else if (((Compare) condition).getOperator() == Condition.Operator.INSTANCE_OF) {
                return ComparatorParser.parseInstanceOfCompare(parseExpression(((Compare) condition).getLeftExpression(), queryEventStreamList, currentStreamReference, processInStreamDefinition, siddhiContext),
                                                               parseExpression(((Compare) condition).getRightExpression(), queryEventStreamList, currentStreamReference, processInStreamDefinition, siddhiContext));
            }
            ...
            for (Expression expression : ((ConditionExtension) condition).getParameters()) {
                expressionExecutors.add(parseExpression(expression, queryEventStreamList, currentStreamReference, processInStreamDefinition, siddhiContext));
            }
    ...
    public static ExpressionExecutor parseExpression(Expression expression,
        ...
        if (expression instanceof Constant) {
            ...
            return new VariableExpressionExecutor(((Variable) expression).getStreamId(), ((Variable) expression).getAttributeName(), ((Variable) expression).getPosition(), queryEventStreamList, currentStreamReference,processInStreamDefinition);

        } else if (expression instanceof Multiply) {
            ExpressionExecutor left = parseExpression(((Multiply) expression).getLeftValue(), queryEventStreamList, currentStreamReference, processInStreamDefinition, siddhiContext);
            ExpressionExecutor right = parseExpression(((Multiply) expression).getRightValue(), queryEventStreamList, currentStreamReference, processInStreamDefinition, siddhiContext);
            ...
            ExpressionExecutor left = parseExpression(((Add) expression).getLeftValue(), queryEventStreamList, currentStreamReference, processInStreamDefinition, siddhiContext);
            ExpressionExecutor right = parseExpression(((Add) expression).getRightValue(), queryEventStreamList, currentStreamReference, processInStreamDefinition, siddhiContext);
            Attribute.Type type = parseArithmeticOperationResultType(left, right);
            switch (type) {
                case INT:
            ...
            ExpressionExecutor left = parseExpression(((Minus) expression).getLeftValue(), queryEventStreamList, currentStreamReference, processInStreamDefinition, siddhiContext);
            ExpressionExecutor right = parseExpression(((Minus) expression).getRightValue(), queryEventStreamList, currentStreamReference, processInStreamDefinition, siddhiContext);
            Attribute.Type type = parseArithmeticOperationResultType(left, right);
            switch (type) {
                case INT:
            ...
            ExpressionExecutor left = parseExpression(((Mod) expression).getLeftValue(), queryEventStreamList, currentStreamReference, processInStreamDefinition, siddhiContext);
            ExpressionExecutor right = parseExpression(((Mod) expression).getRightValue(), queryEventStreamList, currentStreamReference, processInStreamDefinition, siddhiContext);
            Attribute.Type type = parseArithmeticOperationResultType(left, right);
            switch (type) {
                case INT:
            ...
            ExpressionExecutor left = parseExpression(((Divide) expression).getLeftValue(), queryEventStreamList, currentStreamReference, processInStreamDefinition, siddhiContext);
            ExpressionExecutor right = parseExpression(((Divide) expression).getRightValue(), queryEventStreamList, currentStreamReference, processInStreamDefinition, siddhiContext);
            Attribute.Type type = parseArithmeticOperationResultType(left, right);
            switch (type) {
                case INT:
            ...
            for (Expression innerExpression : ((ExpressionExtension) expression).getParameters()) {
                innerExpressionExecutors.add(parseExpression(innerExpression, queryEventStreamList, currentStreamReference, processInStreamDefinition, siddhiContext));
            }
