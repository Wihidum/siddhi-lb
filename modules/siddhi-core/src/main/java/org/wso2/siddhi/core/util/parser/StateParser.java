    ...
    public static List<PatternState> convertToPatternStateList(List<State> stateList) {
        ...
        for (int i = 0, stateListSize = stateList.size(); i < stateListSize; i++) {
            ...
            if (state instanceof AndState) {
                ...
                patternState = (new CountPatternState(state.getStateNumber(), state.getTransformedStream(), ((CountState) state).getMin(), ((CountState) state).getMax()));
            } else {
    ...
    private static List<State> identifyStates(PatternElement patternElement, List<State> stateList,
        ...
        if (patternElement instanceof BasicStream) {
            ...
            if (((LogicalElement) patternElement).getType() == LogicalElement.Type.OR) {
                stateList.add(new OrState(stateNumber.getNumber(), ((LogicalElement) patternElement).getTransformedStream1(), stateNumber.getNumber() + 1));
                addStateAsNext(stateList, stateNumber, perv);
                stateNumber.increment();
                stateList.add(new OrState(stateNumber.getNumber(), ((LogicalElement) patternElement).getTransformedStream2(), stateNumber.getNumber() - 1));
                //addStateAsNext(stateList, stateNumber, perv);
                stateNumber.increment();
            } else {//AND
                stateList.add(new AndState(stateNumber.getNumber(), ((LogicalElement) patternElement).getTransformedStream1(), stateNumber.getNumber() + 1));
                addStateAsNext(stateList, stateNumber, perv);
                stateNumber.increment();
                stateList.add(new AndState(stateNumber.getNumber(), ((LogicalElement) patternElement).getTransformedStream2(), stateNumber.getNumber() - 1));
                //addStateAsNext(stateList, stateNumber, perv);
                stateNumber.increment();
            }
            perv.clear();
            ...
            stateList.add(new CountState(stateNumber.getNumber(), ((CountElement) patternElement).getTransformedStream(), ((CountElement) patternElement).getMinCount(), ((CountElement) patternElement).getMaxCount()));
            addStateAsNext(stateList, stateNumber, perv);
            stateNumber.increment();
            perv.clear();
            perv.add(stateNumber.getNumber() - 1);
...
//            }
    ...
    public static List<SequenceState> convertToSequenceStateList(List<State> stateList) {
        ...
        for (int i = 0, stateListSize = stateList.size(); i < stateListSize; i++) {
            ...
            if (state instanceof OrState) {
                ...
                sequenceState = (new CountSequenceState(state.getStateNumber(), state.getTransformedStream(), ((CountState) state).getMin(), ((CountState) state).getMax()));
            } else {
    ...
    private static List<State> identifyStates(SequenceElement sequenceElement,
        ...
        if (sequenceElement instanceof BasicStream) {
            ...
            stateList.add(new OrState(stateNumber.getNumber(), ((OrElement) sequenceElement).getTransformedStream1(), stateNumber.getNumber() + 1));
            addStateAsNext(stateList, stateNumber, perv);
            stateNumber.increment();
            stateList.add(new OrState(stateNumber.getNumber(), ((OrElement) sequenceElement).getTransformedStream2(), stateNumber.getNumber() - 1));
            //addStateAsNext(stateList, stateNumber, perv);
            stateNumber.increment();
            ...
            stateList.add(new CountState(stateNumber.getNumber(), ((RegexElement) sequenceElement).getTransformedStream(), ((RegexElement) sequenceElement).getMinCount(), ((RegexElement) sequenceElement).getMaxCount()));
            addStateAsNext(stateList, stateNumber, perv);
            stateNumber.increment();
            perv.clear();
            perv.add(stateNumber.getNumber() - 1);
